...
describe('ArticuloRegular', () => {
    ...
    beforeEach(() => {
        ...
        autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
    });
    ...
    test('Crear un artículo regular con resumen de más de 300 palabras debería lanzar un error', () => {
        ...
        expect(() => new ArticuloRegular('Título 1', 'http://archivo1.com', resumenLargo, [autor1])).toThrow("El resumen no puede tener más de 300 palabras.");
    });

    test('El puntaje final del artículo es el promedio de las revisiones', () => {
        const articulo = new ArticuloRegular("Título 1", "http://archivo1.com", "Resumen del artículo 1", [autor1], autor1);
...
describe('ArticuloPoster', () => {
    ...
    test('Crear un artículo poster con autor encargado no válido, lanza exception.', () => {
        const autor2 = new Autor('Pedro Rodríguez', 'UNLP', 'pedro@unlp.edu', 'password123');
        const articulo = new ArticuloPoster('Título 2', 'http://archivo2.com', 'http://fuentes.com', [autor1]);

        expect(() => new ArticuloPoster('Título 2', 'http://archivo2.com', 'http://fuentes.com', [autor1], autor2))
    ...
    test('Crear un artículo poster, luego cambia el autor encargado a uno válido y a uno no válido', () => {
        const autor2 = new Autor('Carlos Pérez', 'UNLP', 'carlos@unlp.edu', 'password789');
        const autor3 = new Autor('Juan Pérez', 'UNLP', 'juan@unlp.edu', 'password555');
        const articulo = new ArticuloPoster('Título 2', 'http://archivo2.com', 'http://fuentes.com', [autor1, autor2]);
        ...
        expect(() => articulo.setAutorEncargado(autor3)).toThrow('El autor encargado debe estar en la lista de autores.');
        expect(articulo.autorEncargado.nombreCompleto).toBe('Carlos Pérez');
    });
});
...
describe('Articulo (Abstract)', () => {
    test('No se puede instanciar directamente la clase abstracta Articulo', () => {
        const autor = new Autor('Carlos Pérez', 'UNLP', 'carlos@unlp.edu', 'password789');
        expect(() => new Articulo('Título 2', 'http://archivo2.com', 'Tipo', [autor]))
            .toThrow('Cannot instantiate abstract class.');
    });

    test('No se puede instanciar un Articulo sin autores válidos', () => {
        expect(() => new ArticuloRegular('Título 2', 'http://archivo2.com', 'Tipo', [null]))
            .toThrow('Debe haber al menos un autor válido.');
        
        expect(() => new ArticuloRegular('Título 2', 'http://archivo2.com', 'Tipo', []))
            .toThrow('Debe haber al menos un autor válido.');
    });

    test('El autor encargado debe estar en la lista de autores', () => {
        const autor1 = new Autor('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
        const autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
        expect(() => new ArticuloPoster('Título 2', 'http://archivo2.com', 'Tipo', [autor1, autor2], new Autor('No Existe', 'UNLP', 'no@unlp.edu', 'password000')))
            .toThrow('El autor encargado debe estar en la lista de autores.');
    });
    ...
    test('Si no se especifica un autor encargado, se usa el primer autor', () => {
        const autor1 = new Autor('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
        const autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
        const articulo = new ArticuloPoster('Título 2', 'http://archivo2.com', 'Tipo', [autor1, autor2]);
    ...
    test('Agregar un observador y notificar a todos los observadores', () => {
        ...
        const articulo = new ArticuloRegular('Título 2', 'http://archivo2.com', 'Tipo', [autor]);
    ...
    test('Agregar un autor solo si no existe en la lista', () => {
        const autor1 = new Autor('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
        const autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
        const nuevoAutor = new Autor('Juan Pérez', 'UNLP', 'juan@unlp.edu', 'password321');

        const articulo = new ArticuloPoster('Título 2', 'http://archivo2.com', 'Tipo', [autor1]);
    ...
    test('Establecer un autor encargado válido', () => {
        const autor1 = new Autor('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
        const autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
        const articulo = new ArticuloRegular('Título 2', 'http://archivo2.com', 'Tipo', [autor1, autor2]);
    ...
    test('No se puede establecer un autor encargado no válido', () => {
        const autor1 = new Autor('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
        const autor2 = new Autor('Luis Fernández', 'UNLP', 'luis@unlp.edu', 'password123');
        const autorInvalido = new Autor('Pedro Rodríguez', 'UNLP', 'pedro@unlp.edu', 'password789');
        const articulo = new ArticuloRegular('Título 2', 'http://archivo2.com', 'Tipo', [autor1, autor2]);

        expect(() => articulo.setAutorEncargado(autorInvalido)).toThrow('El autor encargado debe estar en la lista de autores.');
    });
