...
test('Aceptar y seleccionar artículos regulares y de posters usando diferentes estrategias.', () => {
  const autor = new Usuario('Ana Gómez', 'UNLP', 'ana@unlp.edu', 'password456');
  const articuloRegular1 = new ArticuloRegular('Título Regular 1', 'http://archivo1.com', 'Resumen del artículo regular 1', [autor], autor, 3);
  const articuloRegular2 = new ArticuloRegular('Título Regular 2', 'http://archivo2.com', 'Resumen del artículo regular 2', [autor], autor, 1);
  const articuloPoster1 = new ArticuloPoster('Título Poster 1', 'http://archivo3.com', 'Resumen del artículo poster 1', [autor], autor, 4);
  const articuloPoster2 = new ArticuloPoster('Título Poster 2', 'http://archivo4.com', 'Resumen del artículo poster 2', [autor], autor, 2);
  ...
  sesion.setMetodoSeleccion(metodoSeleccionRegulares, 'Regular');
  sesion.setMetodoSeleccion(metodoSeleccionPosters, 'Poster');

  const seleccionados = sesion.seleccionarArticulos();

  expect(seleccionados.length).toBe(2);
  expect(seleccionados).toContain(articuloRegular1);
  expect(seleccionados).not.toContain(articuloRegular2);
